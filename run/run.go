...
func Run(pb playbook.Playbook, consulAddress string, sqlroot string, fromStep string, dryRun bool) []TargetStatus {
	...
	readySteps, err := loadSteps(pb.Steps, sqlroot, consulAddress, pb.Variables)
	if err != nil {
		allStatuses := make([]TargetStatus, 0)
		for _, tgt := range pb.Targets {
			status := loadQueryFailed(tgt.Name, err)
			allStatuses = append(allStatuses, status)
		}
		return allStatuses
	}
...
func loadSteps(steps []playbook.Step, sqlroot string, consulAddress string, variables map[string]interface{}) ([]ReadyStep, error) {
	sCount := len(steps)
...
// Helper for a fromStep not found error
func loadQueryFailed(targetName string, err error) TargetStatus {
	errs := []error{fmt.Errorf("%s: %s", ERROR_QUERY_FAILED_INIT, err)}
	return TargetStatus{
		Name:   targetName,
		Errors: errs,
		Steps:  nil,
