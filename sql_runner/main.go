...
func main() {
	...
	statuses := Run(*pb, sp, options.fromStep, options.runQuery, options.dryRun, options.fillTemplates, options.dropOutput)
...
func processFlags() Options {
	...
	sr, err := resolveSqlRoot(options.sqlroot, options.playbook, options.consul, options.consulLock)
	if err != nil {
...
func resolveSqlRoot(sqlroot string, playbookPath string, consulAddress string, consulLock bool) (string, error) {
	...
	consulErr3 := fmt.Errorf("Cannot use %s option with -consulLock argument", sqlroot)
