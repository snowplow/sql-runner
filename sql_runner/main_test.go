...
func TestResolveSqlRoot(t *testing.T) {
	...
	str, err := resolveSQLRoot(sqlrootBinary, "../integration/resources/good-postgres.yml", "", false)
	assert.NotNil(str)
	assert.Nil(err)
	str, err = resolveSQLRoot(sqlrootBinary, "../integration/resources/good-postgres.yml", "localhost:8502", false)
	assert.NotNil(str)
	assert.NotNil(err)
	assert.Equal("", str)
	assert.Equal("Cannot use BINARY option with -consul argument", err.Error())

	str, err = resolveSQLRoot(sqlrootPlaybook, "../integration/resources/good-postgres.yml", "", false)
	assert.NotNil(str)
	assert.Nil(err)
	assert.True(strings.HasSuffix(str, "/integration/resources"))
	str, err = resolveSQLRoot(sqlrootPlaybook, "../integration/resources/good-postgres.yml", "localhost:8502", false)
	assert.NotNil(str)
	assert.Nil(err)
	assert.True(strings.HasSuffix(str, "/integration/resources"))

	str, err = resolveSQLRoot(sqlrootPlaybookChild, "../integration/resources/good-postgres.yml", "", false)
	assert.NotNil(str)
	assert.NotNil(err)
	assert.Equal("", str)
	assert.Equal("Cannot use PLAYBOOK_CHILD option without -consul argument", err.Error())
	str, err = resolveSQLRoot(sqlrootPlaybookChild, "../integration/resources/good-postgres.yml", "localhost:8502", false)
	assert.NotNil(str)
	assert.Nil(err)
